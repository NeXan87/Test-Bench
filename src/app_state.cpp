...
inline unsigned long mapFast(int x, int in_min, int in_max, unsigned long out_min, unsigned long out_max) {
    ...
    int inSpan = in_max - in_min;
    int xRel = x - in_min;

    long outMinSigned = (long)out_min;
    long outSpan = (long)out_max - outMinSigned;

    double ratio = (double)xRel / (double)inSpan;
    double value = ratio * (double)outSpan + (double)outMinSigned;
...
void app_state_update() {
    ...
    int adc_on1 = calibration_getCalibratedADC(analogRead(POT_ON1_PIN), 0);
    int adc_d1 = calibration_getCalibratedADC(analogRead(POT_DELAY1_PIN), 1);
    int adc_on2 = calibration_getCalibratedADC(analogRead(POT_ON2_PIN), 2);
    int adc_d2 = calibration_getCalibratedADC(analogRead(POT_DELAY2_PIN), 3);
    int adc_cycles = calibration_getCalibratedADC(analogRead(POT_CYCLES_PIN), 4);
